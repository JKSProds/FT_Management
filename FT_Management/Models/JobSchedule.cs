...
public class HelloWorldJob : IJob
{
    ...
    public Task Execute(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            SmtpClient mySmtpClient = new SmtpClient("iberweb-5a.ibername.com");

            // set smtp-client with basicAuthentication
            mySmtpClient.UseDefaultCredentials = false;
            System.Net.NetworkCredential basicAuthenticationInfo = new
               System.Net.NetworkCredential("jmonteiro@food-tech.pt", "FoodT_02");
            mySmtpClient.Credentials = basicAuthenticationInfo;

            // add from,to mailaddresses
            MailAddress from = new MailAddress("jmonteiro@food-tech.pt", "FT - Management");
            MailAddress to = new MailAddress("assistecnica@food-tech.pt", "Assistência Ténica");
            ...
            var pdf = new MemoryStream(dbContext.PreencherFormularioFolhaObra(dbContext.ObterFolhaObra(1)).ToArray());
            using (Attachment att = new Attachment(pdf, "FolhaObra_1", System.Net.Mime.MediaTypeNames.Application.Pdf))
            {
